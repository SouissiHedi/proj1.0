...
<body>
    ...
    {
        $argent_id=$_GET['ID'];
        $query = "SELECT * FROM argent WHERE ID=:arg_id LIMIT 1";
        $statement = $conn->prepare($query);
        $data = [':arg_id' => $argent_id];
    ...
    ?>
        ...
        <form id="login_form" class="form_class" action="code1.php" method="post">
            ...
            <div class="form_div">
                ...
                include('message.php'); 
                ?>
                <input type='hidden'  class="field_class" name="argent_id" value="<?=$result->ID;?>"   required>
                
                <label>adresse e-mail</label>
                <input id="mail" class="field_class" name="mail" value="<?=$result->mail;?>"  type="text"  required>
                <label>prix a envoyer</label>
                <input class="field_class" name="prix"  value="<?=$result->prix;?>" type="text" placeholder="" autofocus>
                <label>envoyer a</label>
                <input id="aqui" class="field_class" name="aqui"  value="<?=$result->aqui;?>" type="text" placeholder="">
